...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    count2=0;
    count3=0;
    rx_cfo_est_ltsSTS=zeros(1200,1);
    maxProb=zeros(1200,1);
    poll=zeros(1200,1);
for trailtimes=1:1
    for n=1:2
        tic 
        CHANNEL         = 1;         % Choose the channel we will receive on

        NUM_SAMPLES     = 2^26;      % Number of samples to request
...
%         end
    ...
    %     end
        ...
        zigstart1=find(zigstart>3600000+zigstart(1));
        zigstart2=find(zigstart>7200000+zigstart(1));
        zigstart3=find(zigstart>10800000+zigstart(1));
        ...
        det_set(:,1)=rx_IQ(zigstart(1):zigstart(1)+16500,1);
        ...
        for j=1:12
            ...
            Prob(1,1)=normpdf(rx_cfo_est_ltsSTS(j,1),-0.0564,0.0108);
            Prob(1,2)=normpdf(rx_cfo_est_ltsSTS(j,1),-0.0810,0.0082);
            Prob(1,3)=normpdf(rx_cfo_est_ltsSTS(j,1),-0.1005,0.0101);
            Prob(1,4)=normpdf(rx_cfo_est_ltsSTS(j,1),-0.0533,0.0075);
            [maxProb(j+12*(n-1),1), poll(j+12*(n-1),1)]=max(Prob);
            numchan1=length(find(poll==1));
            numchan2=length(find(poll==2));
            numchan3=length(find(poll==3));
            numchan4=length(find(poll==4));
            [maxnum,maxlocation(trailtimes)]=max([numchan1,numchan2,numchan3,numchan4]);
        end
        t=toc;
